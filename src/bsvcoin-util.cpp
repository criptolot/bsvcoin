...
static void SetupBsvcoinUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bsvcoin-util utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  bsvcoin-util [options] [commands]  Do stuff\n";
