...
static bool AppInit(int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/bsvcoin.conf are parsed in qt/bsvcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  bsvcoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bsvcoind -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for bsvcoind but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect bsvcoind signal handlers
    noui_connect();
