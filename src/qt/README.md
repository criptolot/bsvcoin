This directory contains the source code for the Bsvcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Bsvcoin UI.
...
- ClientModel is used by the main application `bsvcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bsvcoin:... link from a browser).
...
* UI elements like BsvcoinAmountField, which inherit from QWidget.
* `bsvcoinstrings.cpp`: automatically generated
* `bsvcoinunits.(h/cpp)`: BSV / mBSV / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bsvcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bsvcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bsvcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bsvcoin-qt`
9. You're all set! Start developing, building, and debugging the Bsvcoin Core GUI
