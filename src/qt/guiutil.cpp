...
#include <qt/bsvcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bsvcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BsvcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new BsvcoinAddressCheckValidator(parent));
}

bool parseBsvcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bsvcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("bsvcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BsvcoinUnits::parse(BsvcoinUnits::BSV, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBsvcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBsvcoinURI(uriInstance, out);
}

QString formatBsvcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bsvcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BsvcoinUnits::format(BsvcoinUnits::BSV, info.amount, false, BsvcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBsvcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BSVCOIN_CONF_FILENAME));
    ...
    /* Open bsvcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bsvcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bsvcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Bsvcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Bsvcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bsvcoin.desktop";
    return GetAutostartDir() / strprintf("bsvcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bsvcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bsvcoin\n";
        else
            optionFile << strprintf("Name=Bsvcoin (%s)\n", chain);
