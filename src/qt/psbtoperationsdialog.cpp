...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = BsvcoinUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(BsvcoinUnits::formatWithUnit(BsvcoinUnits::BSV, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(BsvcoinUnits::formatWithUnit(BsvcoinUnits::BSV, *analysis.fee));
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(BsvcoinUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
