...
const char * const BSVCOIN_CONF_FILENAME = "bsvcoin.conf";
const char * const BSVCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bsvcoin/bsvcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bsvcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BSVCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bsvcoin
    // macOS: ~/Library/Application Support/Bsvcoin
    // Unix-like: ~/.bsvcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bsvcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bsvcoin";
#else
    // Unix-like
    return pathRet / ".bsvcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BSVCOIN_CONF_FILENAME);
