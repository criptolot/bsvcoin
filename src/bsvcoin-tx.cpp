...
static void SetupBsvcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bsvcoin-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  bsvcoin-tx [options] <hex-tx> [commands]  Update hex-encoded bsvcoin transaction\n"
                "or:     bsvcoin-tx [options] -create [commands]   Create hex-encoded bsvcoin transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bsvcoin transaction
