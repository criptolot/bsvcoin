...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BSV per 1,000
  vbytes (BSV/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BSV/kB
  to sat/vB and the help documentation in BSV/kB is updated to BSV/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BSV/kvB to sat/vB and the
  ...
  difference between BSV/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BSV/kvB rather than sat/vB should raise an
...
- #18297 Use pkg-config in BSVCOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
