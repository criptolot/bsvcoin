...
- If a Tor hidden service is being used, Bitcoin Core will be bound to
  the standard port 9444 even if a different port is configured for
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BSV), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BSV per 1,000
...
- #16521 Use the default maxfeerate value as BSV/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as BSV/kB (jonatack)
...
- #15683 Comment for seemingly duplicate LIBBSVCOIN_SERVER (Bushstar)
