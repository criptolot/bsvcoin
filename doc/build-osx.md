...
This guide describes how to build bsvcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Bsvcoin Core from source.
...
### 4. Clone Bsvcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Bsvcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `bsvcoind` or  `bsvcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/bsvcoin/bsvcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Bsvcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/bsvcoin/bsvcoin/issues/7714).
...
You can deploy a `.dmg` containing the Bsvcoin Core application using `make deploy`.
...
## Building Bsvcoin Core

### 1. Configuration

There are many ways to configure Bsvcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Bsvcoin Core:
...
## Running Bsvcoin Core

Bsvcoin Core should now be available at `./src/bsvcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/bsvcoin-qt`.

The first time you run `bsvcoind` or `bsvcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Bsvcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Bsvcoin"

touch "/Users/${USER}/Library/Application Support/Bsvcoin/bsvcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Bsvcoin/bsvcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Bsvcoin/debug.log
```

## Other commands:

```shell
./src/bsvcoind -daemon      # Starts the bsvcoin daemon.
./src/bsvcoin-cli --help    # Outputs a list of command-line options.
./src/bsvcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/bsvcoin-qt -server # Starts the bsvcoin-qt server mode, allows bsvcoin-cli control
```
