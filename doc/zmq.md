...
The Bsvcoin Core daemon can be configured to act as a trusted "border
router", implementing the bsvcoin wire protocol and relay, making
...
The ZeroMQ feature in Bsvcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bsvcoind:
...
For instance:
    ...
    $ bsvcoind -zmqpubhashtx=tcp://127.0.0.1:29443 \
               -zmqpubhashtx=tcp://192.168.1.2:29443 \
               -zmqpubrawtx=ipc:///tmp/bsvcoind.tx.raw \
...
These options can also be provided in bsvcoin.conf.
...
From the perspective of bsvcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bsvcoind directly. Furthermore, no information is
...
using. Bsvcoind appends an up-counting sequence number to each
