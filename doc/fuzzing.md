# Fuzzing Bsvcoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Bsvcoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/bsvcoin/bsvcoin
$ cd bsvcoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/bsvcoin/bsvcoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/bsvcoin/bsvcoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/bsvcoin/bsvcoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/bsvcoin/bsvcoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`bsvcoin-core/qa-assets`](https://github.com/bsvcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`bsvcoin-core/qa-assets`](https://github.com/bsvcoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/bsvcoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`bsvcoin-core/qa-assets`](https://github.com/bsvcoin-core/qa-assets) repo.

Every single pull request submitted against the Bsvcoin Core repo is automatically tested against all inputs in the [`bsvcoin-core/qa-assets`](https://github.com/bsvcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Bsvcoin Core more robust.
...
with certain assembly code from Bsvcoin Core's code. See [developer notes on sanitizers](https://github.com/bsvcoin/bsvcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Bsvcoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Bsvcoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Bsvcoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Bsvcoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Bsvcoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Bsvcoin
Core without having to write any custom fuzzing harness. The `bsvcoind` server
...
```sh
$ mkdir bsvcoin-honggfuzz-p2p/
$ cd bsvcoin-honggfuzz-p2p/
$ git clone https://github.com/bsvcoin/bsvcoin
$ cd bsvcoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/bsvcoind.cpp b/src/bsvcoind.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/bsvcoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
