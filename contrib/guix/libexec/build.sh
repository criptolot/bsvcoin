...
make -C depends --jobs="$JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform bsvcoin-linux-g++' \
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} BSVCOIN_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Bsvcoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Bsvcoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
