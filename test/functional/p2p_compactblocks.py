...
# TestP2PConn: A peer we use to send messages to bsvcoind, and store responses.
...
class CompactBlocksTest(BsvcoinTestFramework):
    ...
    # This test actually causes bsvcoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that bsvcoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for bsvcoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # bsvcoind will not send blocktxn responses for blocks whose height is
