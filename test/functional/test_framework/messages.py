...
"""Bsvcoin test framework primitive and message structures
...
COIN = 100000000  # 1 BSV in satoshis
...
# Objects that map to bsvcoind objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/bsvcoin/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # Bsvcoin Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in bsvcoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in bsvcoind
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in bsvcoind indicates these should be deserialized as blocks
