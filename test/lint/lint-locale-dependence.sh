...
# Be aware that bsvcoind and bsvcoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in bsvcoind.
#
...
# C locale.
#
# In contrast, bsvcoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where bsvcoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of bsvcoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in bsvcoind but
# "1,230000" in bsvcoin-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/bsvcoin-tx.cpp.*stoul"
    "src/bsvcoin-tx.cpp.*trim_right"
